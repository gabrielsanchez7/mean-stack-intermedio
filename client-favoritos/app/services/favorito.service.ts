...
export class FavoritoService {
	...
	constructor(private _http: Http) {
		this.url = 'http://localhost:2717/api/';
	}
	...
	editFavorito(id: string, favorito: Favorito){
		...
		return this._http.put(this.url + 'update-favorito/' + id, params, {headers: headers})
			.map(res => res.json());
	}

	deleteFavorito(id: string){
		return this._http.delete(this.url + 'delete-favorito/' + id)
			.map(res => res.json());
	}
